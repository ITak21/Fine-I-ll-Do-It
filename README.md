...
프로젝트 이름은 `CardPattern`이지만, 실제 앱 이름은 `FIDI (Fine, I'll Do It)`입니다.
...
## GitHub에 올릴 때 권장하는 방식

1. 소스 코드를 저장소에 올립니다.
2. `README.md`와 스크린샷을 정리합니다.
3. `npm run tauri build`로 설치 파일을 만듭니다.
4. GitHub Releases를 생성합니다.
5. `.exe` 또는 `.msi` 파일을 Release Asset으로 첨부합니다.

그러면 저장소 방문자는 소스를 직접 빌드하지 않아도 Releases 탭에서 바로 다운로드할 수 있습니다.
